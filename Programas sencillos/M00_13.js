...
let cantidad_invertida = prompt("Cantidad invertida: ");
let P = parseFloat(cantidad_invertida);
let interes = prompt("Interés anual: ");
let r = parseFloat(interes) / 100;
let anios = prompt("Años transcurridos: ");
let t = parseInt(anios);
let n_veces = prompt("Número de veces que se acumula el interés por año: ");
let n = parseInt(n_veces);

// Cálculos
let A = P * Math.pow((1 + (r / n)), (n * t));
let cantidad_ganada = A;

// Muestra de resultados
console.log(`Tras ${anios} años de inversión al ${interes} %, calculando cada ${n_veces} el interés, su cantidad debe ser: ${cantidad_ganada.toFixed(2)} €`);
