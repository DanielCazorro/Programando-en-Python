...
while pizza <= 0:
    try:
        pizza = input("¿Cuántas pizzas se han comprado?: ")
        pizza = int(pizza)
    except:
        print("Debe introducir un número entero y positivo de pizzas.")
        pizza = 0

personas = 0
while personas <= 0:
    try:
        personas = input("¿Cuántas personas hay?: ")
        personas = int(personas)
    except:
        print("Debe introducir un número entero y positivo de personas.")
        personas = 0
...
sobrante = (porciones*pizza) % personas

# Resultados:
print(f"Hay {personas} personas y {pizza} pizzas.\nCada persona toma {pizza} porciones.\nSobran {sobrante} porciones")
